...
Page({
  ...
  onLoad(options) {
    ...
    this.setData({
      ...
      best_score: wx.getStorageSync('best_score') || 0
    });
  },
  ...
  touchEnd: function (events) {
    ...
    if (Math.max(Math.abs(dx), Math.abs(dy)) > 10) {
      ...
      var best_score = wx.getStorageSync('best_score') || 0;
      if (data.score > best_score) {
        wx.setStorageSync('best_score', data.score);
      }
      ...
      this.updateView({
        ...
        won: data.won,
        score: data.score,
        best_score: Math.max(best_score, data.score)
      });
    }
  }
});